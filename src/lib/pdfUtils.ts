...
export const generateSalaryPDF = (slip: any) => {
  ...
  (doc as any).autoTable({
    ...
    body: [
      ['Basic Salary', `PKR ${slip.BasicSalary.toLocaleString()}`],
      ['Allowances', `PKR ${slip.Allowances.toLocaleString()}`],
      ['Deductions', `-PKR ${slip.Deductions.toLocaleString()}`],
      [{ content: 'Net Payable', styles: { fontStyle: 'bold' } }, { content: `PKR ${slip.NetSalary.toLocaleString()}`, styles: { fontStyle: 'bold' } }],
    ],
    theme: 'striped',
