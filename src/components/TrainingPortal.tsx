        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {loading ? (
            ...
            <motion.div 
              key={mod.ModuleID}
              whileHover={{ y: -4 }}
              className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl flex gap-6 items-start group"
            >
              <div className="w-16 h-16 bg-zinc-950 border border-zinc-800 rounded-xl flex items-center justify-center shrink-0 group-hover:border-orange-600/50 transition-colors">
                <BookOpen className="text-zinc-500 group-hover:text-orange-500 transition-colors" size={24} />
              </div>
              <div className="flex-1 space-y-2">
                <div className="flex justify-between items-start">
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${mod.Mandatory ? 'text-red-500 bg-red-500/10' : 'text-zinc-500 bg-zinc-500/10'}`}>
                    {mod.Mandatory ? 'Mandatory' : 'Optional'}
                  </span>
                ...
                <div className="pt-4 flex items-center justify-between">
                  ...
                  <button className="text-white bg-zinc-800 hover:bg-orange-600 px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2">
